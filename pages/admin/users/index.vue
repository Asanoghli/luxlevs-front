...
<template>
  <div class="w-full h-full max-h-dvh overflow-y-scroll">
    <div class="px-6 bg-gray-700 h-16 flex items-center justify-between">
      <div>
        <button  class="p-3 px-7 bg-red-600 text-white  rounded disabled:bg-red-900 disabled:cursor-not-allowed">DELETE (5)</button>
      </div>
      <div class="flex grow gap-2 justify-center items-center">
...
<!-- If No One Checked-->
      ...
      <tr class="h-14" v-for="user in users" :key="user.id" :class="user.checked ? 'bg-gray-300' : null">
